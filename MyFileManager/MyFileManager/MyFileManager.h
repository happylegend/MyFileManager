...
+(NSString *)createFolder:(NSString *)folderName;      //创建文件夹，返回的是文件夹的完整路径名
+(NSString *)createFile:(NSString *)fileName inFolder:(NSString *)folderName;     //返回文件完整路径名，当folerName为nil时，在默认文件夹中
+(BOOL)readFromFile:(NSString *)fileFullPath toObject:(NSObject *)object;         //从文件中读取内容到字符串，data，array，dictionary中等
